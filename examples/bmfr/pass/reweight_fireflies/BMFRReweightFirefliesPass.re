...
let init = (device, swapChainFormat, state) => {
  ...
  let (cascadeBuffersBufferSize, cascadeBuffersBuffer) =
    BMFRBuffer.ReweightFireflies.CascadeBuffers.unsafeGetBufferData(state);
  let (paramBufferData, paramBuffer) =
    BMFRBuffer.ReweightFireflies.ParamBuffer.unsafeGetBufferData(state);

    Log.print(("paramBufferData:" ,paramBufferData )) |> ignore;
  ...
  let bindGroup =
    ...
    |> Device.createBindGroup({
         ...
         "bindings": [|
           ...
           BindGroup.binding(
             ~binding=1,
             ~buffer=cascadeBuffersBuffer,
             ~offset=0,
             ~size=cascadeBuffersBufferSize,
             (),
           ),
           BindGroup.binding(
             ~binding=2,
             ...
             ~offset=0,
             ~size=
               BMFRBuffer.ReweightFireflies.ParamBuffer.getBufferSize(
                 paramBufferData,
               ),
             (),
           ),
  ...
  let state =
    state |> Pass.ReweightFirefliesPass.setStaticBindGroupData(0, bindGroup);

  let baseShaderPath = "examples/bmfr/pass/reweight_fireflies/shaders";
    ...
    |> Device.createShaderModule({
         "code":
           WebGPUUtils.loadShaderFile(
             {j|$(baseShaderPath)/reweight_fireflies.vert|j},
           ),
       });
  let fragmentShaderModule =
    ...
    |> Device.createShaderModule({
         "code":
           WebGPUUtils.loadShaderFile(
             {j|$(baseShaderPath)/reweight_fireflies.frag|j},
           ),
       });
  ...
  state |> Pass.ReweightFirefliesPass.setPipeline(pipeline);
};
...
let execute = (device, queue, swapChain, state) => {
  ...
  let (staticBindGroupData, pipeline) = (
    Pass.ReweightFirefliesPass.unsafeGetStaticBindGroupData(state),
    Pass.ReweightFirefliesPass.unsafeGetPipeline(state),
  );
