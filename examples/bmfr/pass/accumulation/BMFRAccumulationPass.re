...
let init = (device, swapChainFormat, state) => {
  ...
  let (accumulationPixelBufferSize, accumulationPixelBuffer) =
    BMFRBuffer.AccumulationPixelBuffer.unsafeGetBufferData(state);
  let (accumulationCommonDataBufferData, accumulationCommonDataBuffer) =
    BMFRBuffer.AccumulationCommonDataBuffer.unsafeGetBufferData(state);
    ...
    |> Device.createBindGroup({
         ...
         "bindings": [|
           ...
           BindGroup.binding(
             ...
             ~offset=0,
             ~size=
               BMFRBuffer.AccumulationCommonDataBuffer.getBufferSize(
  ...
  let baseShaderPath = "examples/bmfr/pass/accumulation/shaders";
