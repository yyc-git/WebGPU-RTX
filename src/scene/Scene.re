...
let update = (time, window, state) => {
  ...
  let (cameraData, cameraBuffer) =
    ManageBuffer.UniformBuffer.unsafeGetCameraBufferData(state);
  let cameraData =
    cameraData
    |> TypeArray.Float32Array.set(
         0,
         CameraView.unsafeGetViewMatrixInverse(currentCameraView, state),
       )
    |> TypeArray.Float32Array.set(
         CameraView.unsafeGetViewMatrixInverse(currentCameraView, state)
         |> Float32Array.length,
         CameraView.unsafeGetProjectionMatrixInverse(
           currentCameraView,
           state,
         ),
       );
